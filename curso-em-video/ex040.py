# Aquele clássico da Média
'''Crie um programa que leia duas notas de
um aluno e calcule sua média, mostrando uma
mensagem no final, de acordo com a média atingida:
- Média abaixo de 5,0: REPROVADO
- Média entre 5,0 e 6,9: RECUPERAÇÃO
- Média 7,0 ou superior: APROVADO'''

n1 = float(input('Primeira nota: '))
n2 = float(input('Segunda nota: '))
nota = (n1 + n2) / 2
if nota > 10 or nota < 0:
    print('\033[31m''Valores inválidos! Tente novamente.')
elif nota < 5:
    print('NOTA FINAL: \033[31m''{}'.format(nota))
    print('\033[1:31m''REPROVADO''\033[m')
elif 5 < nota < 6.9:
    print('NOTA FINAL: \033[1:33m''{}'.format(nota))
    print('\033[1:33m''RECUPERAÇÃO''\033[m')
elif nota >= 7:
    print('NOTA FINAL: \033[1:32m''{}'.format(nota))
    print('\033[1:32m''APROVADO''\033[m')
