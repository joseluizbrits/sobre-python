...
def gravarDados():
    if tb_nome.get() != '':
        ...
        vquery = f"INSERT INTO tb_contatos (N_NOMECONTATO, T_TELEFONECONTATO, T_EMAILCONTATO, T_OBS) VALUES ('{vnome}', '{vfone}', '{vemail}', '{vobs}')"
        banco.dml(vquery)
        tb_nome.delete(0, END)
        tb_fone.delete(0, END)
        tb_email.delete(0, END)
        tb_obs.delete('1.0', END)
        print('\033[32m''Dados gravados com sucesso')
